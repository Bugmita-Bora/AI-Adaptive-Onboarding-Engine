...
const LINES = [
  { key: 0, text: 'Your personalized', speed: 52 },
  { key: 1, text: 'learning path.',    speed: 68 },
  { key: 2, text: 'In seconds.',       speed: 76 },
];
...
export default function Hero() {
  ...
  const startTyping = (li = 0, ci = 0, state = ['', '', '']) => {
    ...
    if (ci <= text.length) {
      ...
      timerRef.current = setTimeout(() => startTyping(li, ci + 1, next), speed + Math.random() * 28 - 10);
    } else {
      timerRef.current = setTimeout(() => startTyping(li + 1, 0, state), li < LINES.length - 1 ? 160 : 0);
    }
  };
  ...
  return (
    <div className="hero">
      ...
      <p className={`hero-sub${done ? ' visible' : ''}`}>
        Upload your resume and a job description. Our AI maps exactly where
        you stand, identifies the gaps, and builds a personalized path to
        role readiness.
      </p>
